...
md.jsModule = function(ittfDocumentPathOrContext,context,callback) {
	console.log('md.jsModule', ittfDocumentPathOrContext)
	context.__productionManager = createProductionManager('stefi', 'admin', {});
    if (verify.isString(ittfDocumentPathOrContext)) {
        // async load the wizziModelInstance
...
function createProductionManager(user, subscription, globalContext) {
	var pman = new md.ProductionManager({});
	pman.aclStat(new md.acl.AclStat(user, subscription));
	pman.globalContext(globalContext);
	return pman
}
