...
function loadFromIttfModel(ittfModel,ittfDocumentUri,request,callback) {
    ...
    if (ittfModel.nodes.length == 0) {
        ...
        try {
            ...
            var error = new errors.WizziModelLoadError(ex.message + '\nIn site Factory, calling loadFromNode.', ittfDocumentUri);
            fail.warn(error.message);
            return callback(error);
        }
    }
    ...
    if ((request.dumpAll || request.dumpModel) && sitemodel.toJson && file.isFilePath(ittfDocumentUri)) {
        // dump for debug
        var ittfModelDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
        file.write(ittfModelDump, stringify(sitemodel.toJson(), null, 2));
    }
    ...
    sitemodel.wzInitializeAsync(ctx, function(err,result) {
        ...
        if ((request.dumpAll || request.dumpModelAfterInitializeAsync) && sitemodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
