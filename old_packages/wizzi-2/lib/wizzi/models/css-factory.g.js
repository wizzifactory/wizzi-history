...
function loadFromIttfModel(ittfModel,ittfDocumentUri,request,callback) {
    ...
    if ((request.dumpAll || request.dumpModel) && cssmodel.toJson && file.isFilePath(ittfDocumentUri)) {
        // dump for debug
    ...
    cssmodel.wzInitializeAsync(ctx, function(err,result) {
        ...
        if ((request.dumpAll || request.dumpModelAfterInitializeAsync) && cssmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
