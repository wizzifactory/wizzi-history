/*
    ...
    primary source IttfDocument: c:\my\wizzi\v5\apps\wizzi-app\wizzi\ittf\src\components\routes\wizzikernelpackage.js.ittf
...
import {getKernelPackage, getIttfDocument, getMTreeDebugInfo, generateDefaultArtifact, generateWfSchema, executeWfJob} from '../../store/actions';
import MyLink from '../mui/AppDrawerNavItem';
const title = 'Kernel sources';
const WF_ACTION = {
...
class WizziKernelPackageContainer extends React.Component {
    state = {
        selectedSchema: null, 
    ...
    componentDidMount() {
        ...
        dispatchGetKernelPackage(packageName);
    }
    ...
    render() {
        ...
        console.log('WizziKernelPackage.match', match);
        if (!!(rootIttfDocuments && rootIttfDocuments.length > 0) == false) {
            if (loadingError) {
                return  (
                        <div>
                            <h2>
...
const WizziKernelPackageStyled = withStyles(styles)(WizziKernelPackageContainer)
;
const WizziKernelPackageContexted = withPageInfo(WizziKernelPackageStyled);
const mapDispatchToProps = (dispatch) => {
    return {
            dispatch: dispatch, 
            dispatchGetKernelPackage: (packageName) => {
                console.log('dispatchGetKernelPackage', packageName);
                dispatch(getKernelPackage(packageName));
            }, 
            dispatchGetIttfDocument: (hash) => {
...
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(WizziKernelPackageContexted)
;
