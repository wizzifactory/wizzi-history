/*
    ...
    primary source IttfDocument: c:\my\wizzi\v5\apps\wizzi-app\wizzi\ittf\src\components\routes\wizzischemaeditor.js.ittf
...
class WizziSchemaEditorContainer extends React.Component {
    state = {
        selectedSchema: null, 
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (pageInfo && pageInfo.title !== (this.props.match ? title + ': ' + this.props.match.params.schemaId : title)) {
            pageInfo.setTitle(this.props.match ? title + ': ' + this.props.match.params.schemaId : title);
        }
    }
    componentDidMount() {
        const {
    ...
    render() {
        ...
        if (!!(pluginSchemas || pluginSchemas.length == 0) == false) {
            if (loadingError) {
                return  (
                        <div>
                            <h2>
        ...
        const schemasTIttfDocuments_tree = schemasTIttfDocuments ? fsitem_flat_to_tree(schemasTIttfDocuments, 'schemasT') : {};
        // log 'schemasTIttfDocuments', schemasTIttfDocuments
        // log 'schemasTIttfDocuments_tree', schemasTIttfDocuments_tree
        ...
        const itemTIttfDocuments_tree = schemaItem.tIttfDocuments ? fsitem_flat_to_tree(schemaItem.tIttfDocuments, 't') : {};
        return  (
                        ...
                        <Splitter split="vertical" secondaryInitialSize={ 700 } primaryMinSize={200} secondaryMinSize={700} onResize={ this.handleSplitter1Resize }>
                            <div className={classes.editorbox}>
                                ...
                                <div className={classes.split2_pane1}>
                                    ...
                                    <List>
                                        <MyLink onClick={() => {
                                            this.handleSelectIttfDocument(schemaItem.mainDocument.hash);
                                        }} title={schemaItem.mainDocument.baseName}>
                                        </MyLink>
                                    
                                    </List>
                                
                                    <CollapseListItem item={itemTIttfDocuments_tree} onSelect={this.handleSelectIttfDocumentByItem} onCreateFolder={() => {
                                    }} onCreateFile={() => {
                                    }} onDuplicate={() => {
                                    }} onRename={() => {
                                    }} onDelete={() => {
                                ...
                                
                                {
                                    ...
                                    schemasTIttfDocuments_tree.children && schemasTIttfDocuments_tree.children.length > 0 &&  (
                                        <CollapseListItem item={schemasTIttfDocuments_tree} onSelect={this.handleSelectIttfDocumentByItem} onCreateFolder={() => {
                                        }} onCreateFile={() => {
                                        }} onDuplicate={() => {
                                        }} onRename={() => {
                                        }} onDelete={() => {
...
const WizziSchemaEditorStyled = withStyles(styles)(WizziSchemaEditorContainer)
;
const WizziSchemaEditorContexted = withPageInfo(WizziSchemaEditorStyled);
...
const mapStateToProps = (state) => {
    return {
            ...
            schemasTIttfDocuments: state.wizziPackages.schemasTIttfDocuments, 
...
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(WizziSchemaEditorContexted)
;
