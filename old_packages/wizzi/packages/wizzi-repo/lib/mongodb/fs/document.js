/*
    ...
    primary source IttfDocument: c:\my\wizzi\v5\kernel\wizzi-repo\src\ittf\lib\mongodb\fs\document.js.ittf
*/
'use strict';
...
var Document = (function () {
    ...
    Document.prototype.writeFile = function(filePath, content, callback) {
        ...
        console.log('wizzi-repo.fs.mongodb.document.writeFile.writeFile.init', filePath);
        filePath = normalize(filePath);
        console.log('wizzi-repo.fs.mongodb.document.writeFile.writeFile.normalized', filePath);
        var that = this;
        this.fsCommon.getItemByPath(filePath, function(err, fsitem) {
            if (err) {
                return callback(err);
            ...
            if (fsitem != null) {
                console.log('wizzi-repo.fs.mongodb.document.writeFile.writeFile.1.exists, so update', filePath);
                return that._updateFile(fsitem._id, content, callback);
            }
            else {
                var dirname = path.dirname(filePath);
                console.log('wizzi-repo.fs.mongodb.document.writeFile.writeFile.2.not exists. try get dirname', dirname);
                that.fsCommon.getItemByPath(dirname, function(err, fsitem) {
                    if (err) {
                        return callback(err);
                    }
                    if (fsitem != null) {
                        console.log('wizzi-repo.fs.mongodb.document.writeFile.writeFile.3. dirname exists create file', fsitem._id, dirname, path.basename(filePath));
                        return that._createFile(fsitem._id, dirname, path.basename(filePath), content, callback);
                    }
                    else {
                        console.log('wizzi-repo.fs.mongodb.document.writeFile.writeFile.4.dirname not exists.create dirname', dirname);
                        that.createFolder(dirname, function(err, fsitem) {
                            if (err) {
                                return callback(err);
                            }
                            console.log('wizzi-repo.fs.mongodb.document.writeFile.writeFile.5.dirname created. so create file', fsitem.item._id, dirname, path.basename(filePath));
