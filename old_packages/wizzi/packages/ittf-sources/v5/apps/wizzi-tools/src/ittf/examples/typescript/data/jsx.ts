/*
    ...
    It is meant to be transformed into valid JavaScript, though the semantics of that transformation
...
/*
    ...
    The preserve mode will keep the JSX as part of the output to be further consumed by
    another transform step (e.g.Babel).
    ...
    and the output will have a .js file extension.
    The react-native mode is the equivalent of preserve in that it keeps all JSX, but the output w
    ill instead have a .js file extension.
    ...
    You can specify this mode using either the --jsx command line flag or the corresponding option
...
/* This asserts the variable bar to have the type foo. 
    ...
    combining it with JSX’s syntax would introduce certain parsing difficulties.
    As a result, TypeScript disallows angle bracket type assertions in .tsx files.
    Since the above syntax cannot be used in .tsx files, an alternate type assertion operator
    should be used: as.
...
/*
    ...
    between intrinsic elements and value-based elements.
    Given a JSX expression < expr />, expr may either refer to something intrinsic to the environment
    (e.g.a div or span in a DOM environment) or to a custom component that you’ve created.
    This is important for two reasons:
    For React, intrinsic elements are emitted as strings (React.createElement("div")),
    ...
    Intrinsic element attributes should be known intrinsically whereas components will likely
    want to specify their own set of attributes.
...
/*
    ...
    In TypeScript 2.3, TS introduced type checking of children.children is a special property
    in an element attributes type where child JSXExpressions are taken to be inserted into
    the attributes.
    Similar to how TS uses JSX.ElementAttributesProperty to determine the name of props,
    TS uses JSX.ElementChildrenAttribute to determine the name of children within
    those props.JSX.ElementChildrenAttribute should be declared with a single property. */
declare namespace JSX {
...
/*
    Factory Functions
    The exact factory function used by the jsx: react compiler option is configurable.
    It may be set using either the jsxFactory command line option, or an inline @jsx comment pragma
    to set it on a per-file basis.
    For example, if you set jsxFactory to createElement, <div /> will emit as createElement("div")
    instead of React.createElement("div").
...
/* The factory chosen will also affect where the JSX namespace is looked up 
    ...
    will check for React.JSX before checking for a global JSX.
