/*
    ...
    primary source IttfDocument: c:\my\wizzi\v4\plugins\wizzi-codegen\src\ittf\lib\js_v4\statements\objects.js.ittf
    ...
    cnt.stm.jsPropertyOrValue = function(model, ctx) {
        if (u.hasStatementChildren(model)) {
            ...
            if (u.isParamValue(model) || u.isValue(model)) {
                ...
                var tk,
                    p = lineParser.parseNameValueRaw(model.wzName, model, {objectProperty: true})
                    ;
                if (p.hasValue()) {
                    ctx.write(p.name() + ': ');
                    ctx.write(p.value());
