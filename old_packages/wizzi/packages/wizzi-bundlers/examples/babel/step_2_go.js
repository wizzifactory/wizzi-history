/*
    ...
    primary source IttfDocument: c:\my\wizzi\v5\apps\wizzi-bundlers\wizzi\ittf\examples\babel\step_2_go.js.ittf
*/
'use strict';
...
var babel = function(step_callback) {
    ...
    var wizzi_file_1 = path.join(__dirname, 'step_2', 'file_1.js.ittf');
    var wizzi_file_2 = path.join(__dirname, 'step_2', 'file_2.js.ittf');
    babelLoader.babelTransformWizziFile(wizzi_file_1).then((result) => {
        printValue('babelTransform.wizzi_file_1', result.code);
        babelLoader.babelTransformWizziFile(wizzi_file_2).then((result) => {
            printValue('babelTransform.wizzi_file_2', result.code);
            step_callback(null);
        }).catch((err) => {
            console.log('babelTransform.wizzi_file_2.err', err);
            step_callback(err);
        })
    }).catch((err) => {
        console.log('babelTransform.wizzi_file_1.err', err);
        step_callback(err);
    })
};
...
function heading1(text) {
    ...
    console.log('** level 1 - step 2 - babel - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
