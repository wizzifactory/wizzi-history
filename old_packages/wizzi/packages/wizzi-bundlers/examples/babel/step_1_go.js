/*
    ...
    primary source IttfDocument: c:\my\wizzi\v5\apps\wizzi-bundlers\wizzi\ittf\examples\babel\step_1_go.js.ittf
*/
'use strict';
...
var babel = function(step_callback) {
    ...
    babelLoader.babelTransform(code_1).then((result) => {
        printValue('babelTransform.code_1', result.code);
        babelLoader.babelTransform(code_2).then((result) => {
            printValue('babelTransform.code_2', result.code);
            step_callback(null);
        }).catch((err) => {
            console.log('babelTransform.code_2.err', err);
            step_callback(err);
        })
    }).catch((err) => {
        console.log('babelTransform.code_1.err', err);
        step_callback(err);
    })
};
...
function heading1(text) {
    ...
    console.log('** level 1 - step 1 - babel - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
