...
md.load = function (cnt) {
    cnt.stm.call = function (model, ctx) {
        ...
        if (model.statements.length > 0) {
            ...
            if (u.isTopStatement(model) &&
                u.isDescendentOf(model, 'iif') == false) ctx.w(u.semicolon(name));
        }
    }
    cnt.stm.memberCall = function (model, ctx) {
        ...
        if (model.statements.length > 0) {
            ...
            name = hasParens ? '.' + name : '.' + name + '()';
            ctx.write(name);
            if (u.isTopStatement(model)) ctx.w(u.semicolon(name));
        }
    }
    ...
    function doCallChildStatements(model, name, hasParens, ctx) {
        ...
        for (var i = 0; i < model.statements.length; i++) {
            ...
            if ((u.isCallArgument(item) && childsAreMemberExpr) ||
                    u.isMemberAccess(item)) {
                if (hasParens === false && u.isMemberAccess(item)) ctx.write(')');
                ...
                if (u.isTopStatement(model) &&
                    u.isDescendentOf(model, 'iif') == false &&
                    model.wzElement !== 'decoratorCall' &&
                    !u.parentIs(model, 'arrowfunction')) ctx.w(';');
                else
                    ctx.w();
                return;
            }
        ...
        if (u.isTopStatement(model) &&
            model.wzElement !== 'decoratorCall' &&
            !u.parentIs(model, 'arrowfunction') &&
            u.isDescendentOf(model, 'iif') == false) {
            // console.log('call', model.wzElement, model.wzParent.wzElement, model.wzParent.wzParent ? model.wzParent.wzParent.wzElement : '');
            ctx.w(';');
        }
        else
            ctx.w();
    ...
    cnt.stm.memberAccess = function (model, ctx) {
        // console.log('cnt.stm.memberAccess', model.wzParent, ctx.__jskind);
        if (ctx.__inside_html ||
            model.wzParent.wzElement === 'xreturn' && ctx.__jskind === 'react') {
