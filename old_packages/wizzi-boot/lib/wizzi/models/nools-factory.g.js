...
function loadFromIttfModel(ittfModel,ittfDocumentUri,request,callback) {
    ...
    if ((request.dumpAll || request.dumpModel) && noolsmodel.toJson && pathutil.isFilePath(ittfDocumentUri)) {
        // dump for debug
    ...
    noolsmodel.wzInitializeAsync(ctx, function(err,result) {
        ...
        if ((request.dumpAll || request.dumpModelAfterInitializeAsync) && noolsmodel.toJson && pathutil.isFilePath(ittfDocumentUri)) {
            // dump for debug
