...
function loadFromIttfModel(ittfModel,ittfDocumentUri,request,callback) {
    ...
    if ((request.dumpAll || request.dumpModel) && wizzischemamodel.toJson && pathutil.isFilePath(ittfDocumentUri)) {
        // dump for debug
    ...
    wizzischemamodel.wzInitializeAsync(ctx, function(err,result) {
        ...
        if ((request.dumpAll || request.dumpModelAfterInitializeAsync) && wizzischemamodel.toJson && pathutil.isFilePath(ittfDocumentUri)) {
            // dump for debug
