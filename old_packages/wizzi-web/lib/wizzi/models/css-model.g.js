...
var css = (function (cssBase) {
    ...
    css.prototype.wzInitialize = function(ctx) {
        ...
        if (verify.isObject(this.extends)) {
            var rule;
            var i, i_len=Object.keys(this.extends).length, item;
            for (i=0; i<i_len; i++) {
                item = Object.keys(this.extends)[i];
                rule = this.findRuleBySelector(item);
                if (rule) {
                    var j, j_len=this.extends[item].length, ext;
                    for (j=0; j<j_len; j++) {
...
var rule = (function (cssBase) {
    ...
    rule.prototype.wzVerify = function(ctx) {
        ...
        var i, i_len=this.otherClassSelectors.length, item;
        for (i=0; i<i_len; i++) {
            item = this.otherClassSelectors[i];
            item.wzVerify(ctx);
        }
        var i, i_len=this.otherAmpersandSelectors.length, item;
        for (i=0; i<i_len; i++) {
            ...
            if (parentRuleParts.length > 0) {
                ...
                for (i=0; i<i_len; i++) {
                    ...
                    if (this.wzElement == 'media') {
                        ...
                        var j, j_len=thisRuleParts.length, rp;
                        for (j=0; j<j_len; j++) {
                            rp = thisRuleParts[j];
                            if (rp.indexOf('&') > -1) {
