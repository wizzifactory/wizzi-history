...
function loadFromIttfModel(ittfModel,ittfDocumentUri,request,callback) {
    ...
    if ((request.dumpAll || request.dumpModel) && xpackagemodel.toJson && pathutil.isFilePath(ittfDocumentUri)) {
        // dump for debug
    ...
    xpackagemodel.wzInitializeAsync(ctx, function(err,result) {
        ...
        if ((request.dumpAll || request.dumpModelAfterInitializeAsync) && xpackagemodel.toJson && pathutil.isFilePath(ittfDocumentUri)) {
            // dump for debug
