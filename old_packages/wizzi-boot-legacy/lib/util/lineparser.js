...
md.parse = function (text, node) {
    ...
    for (var i = 0; i < l; i++) {
        ...
        if (token.quote != null) {
            ...
            if (token.text.length > 0 && node) errors.NodeError("Invalid use of quote inside text", node);
            token.quote = ch;
    ...
    return {
        ...
        join: function () {
            ...
            for (var i = 0; i < this.tokens.length; i++) {
                ...
                if (token.kind === 0) {
                    t.push('"' + verify.escapename(token.text) + '"');
                } else {
                    if (handlebar) {
                        t.push('"{{ ' + verify.escapename(token.text) + '}}"');
                    } else {
...
md.parseCategoryNameValue = function (text, node) {
    ...
    return {
        category: function () { return nv1.name(); },
        name: function () { return nv2.name(); },
        value: function () { return nv2.value(); },
        hasValue: function () { return value.length > 0; }
    };
}
