/*
    ...
    primary source IttfDocument: c:\my\wizzi\v4\plugins\wizzi-codegen\src\ittf\lib\js_v4\statements\jsonstatementtree.js.ittf
    utc time: Thu, 16 Nov 2017 11:04:00 GMT
*/
'use strict';
...
function normalizeJSTNode(node, parent) {
    if (node.n === 'function') {
        console.log('jsonStatementTree.function', node)
        node.paramNames = [];
    }
    ...
    for (i = 0; i < i_len; i++) {
        ...
        if (node.n === 'function' && item.n === 'param') {
            console.log('jsonStatementTree.param', item.v)
